...
class HybridAgent:
    ...
    def __init__(
        ...
        # RAG memory
        qd_url = qdrant_url if qdrant_url is not None else os.environ.get("QDRANT_URL","")
        az_ep  = azure_embedding_endpoint if azure_embedding_endpoint is not None else os.environ.get("AZURE_EMBEDDING_ENDPOINT","")
        self._rag = RAGMemory(
            memory_path=str(self.memory_path),
            mode="qdrant" if (qd_url and az_ep) else "bm25",
            collection_name=collection_name,
            qdrant_url=qd_url,
            qdrant_api_key=qdrant_api_key if qdrant_api_key is not None else os.environ.get("QDRANT_API_KEY",""),
            azure_embedding_endpoint=az_ep,
            azure_embedding_key=azure_embedding_key if azure_embedding_key is not None else os.environ.get("AZURE_EMBEDDING_KEY",""),
            k=k,
        )
